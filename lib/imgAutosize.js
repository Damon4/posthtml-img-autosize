        ...
        return function(tree) {
            ...
            tree.match({ tag: 'img' }, img => {
                ...
                const imagePath = translatePath(
                    projectRoot, img.attrs.src, options.questionMarkAsVersion
                );
...
function translatePath(projectRoot, imgPath, isQuestionMarkAsVersion) {
    ...
    let imgProjectPath = path.join(projectRoot, imgPath);

    if (isQuestionMarkAsVersion) {
        // Translat dir/file.jpg?v=.. → dir/file.jpg
        imgProjectPath = imgProjectPath.replace(/\?[^/]*/, '');
    }

    return normalizePath(path.resolve(imgProjectPath));
}
